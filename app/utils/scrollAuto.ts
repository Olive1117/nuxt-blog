 ...
 * @returns 清除监听的方法
 */
const scrollAuto = (pEle: HTMLElement, cEle: HTMLElement, codeMirrorUt: CodeMirrorRef) => {
  const view = codeMirrorUt.view!
  ...
  const buildMap = () => {
    ...
    elesHasLineNumber = Array.from(cEle.querySelectorAll<HTMLElement>(DATA_LINE_SELECTOR))
  ...
  const pEleHandler = () => {
    ...
    if (endBottom >= pMaxScrollLength || endElePos > cMaxScrollLength) {
      ...
      blockHeight =
        cMaxScrollLength - startEleOffetTop + getComputedStyleNum(cEle, 'padding-block-start')
    }
  ...
  const cEleHandler = () => {
    ...
    let eleStartOffsetTop =
      realEleStart === cEle.firstElementChild?.firstElementChild
        ? 0
        : realEleStart.offsetTop - getComputedStyleNum(realEleStart, 'margin-block-start')
