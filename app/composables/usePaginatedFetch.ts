...
export async function fetchAllSequential<T = unknown>(url: string, opt?: FetchOptions): Promise<T[]> {
  const pageSize = opt?.pageSize ?? 50;
  const delay = opt?.delay ?? 0;

  let firstPage: ApiResponse<PageResponse<T>>;
  try {
    firstPage = await $fetch<ApiResponse<PageResponse<T>>>(url, { query: { page: 1, page_size: pageSize } });
  } catch (err) {
  ...
  for (let page = 2; page <= totalPage; page++) {
    try {
      const res = await $fetch<ApiResponse<PageResponse<T>>>(url, { query: { page, page_size: pageSize } });
...
export async function fetchAllParallel<T = unknown>(url: string, opt?: FetchOptions): Promise<T[]> {
  ...
  const promises: Promise<ApiResponse<PageResponse<T>>>[] = [];
  ...
  const worker = async () => {
    while (pageQueue.length > 0) {
      ...
      try {
        ...
        throw new Error(`第 ${page} 页并行加载失败: ${err instanceof Error ? err.message : String(err)}`);
      }
    }
  };
